<?php
// Server-side render for lc-rental-properties
...
$query_args = function_exists( 'lc_build_fudo_query_args' )
	...
	: array(
		...
		'posts_per_page' => $items,
		'meta_query'     => array(
			array( 'key' => 'kbn', 'value' => 'rent', 'compare' => '=' ),
		),
	);

$query = new WP_Query( $query_args );

echo '<section class="lc-section-bukken lc-rental-properties">';
if ( $title !== '' ) {
	echo '<h2 class="lc-section__ttl"><small>RENT / 賃貸</small>' . esc_html( $title ) . '</h2>';
}
...
if ( $query->have_posts() ) :
	...
	while ( $query->have_posts() ) : $query->the_post();
		if ( function_exists( 'lc_render_property_block_card' ) ) {
			lc_render_property_block_card( get_the_ID(), 'tag-rent', '賃貸', $display_items, $button_text, $newup_days );
		} elseif ( function_exists( 'lc_render_fudo_card' ) ) {
			lc_render_fudo_card( 'tag-rent', '賃貸' );
		} else {
	...
	echo '<div class="lc-bukken-empty"><p>賃貸物件は現在ありません。</p></div>';
endif;
